...
class ClassSchedule extends Model
{
    ...
    protected $fillable = ['student_id','teacher_id', 'subject_id','date','time_start','time_end','mode','status'];
    ...
    protected static function booted()
    {
        ...
        static::updated(function (ClassSchedule $schedule) {
            ...
            self::handleChangeStartTime($schedule);
        });
    }
    ...
    protected static function handleChangeStartTime(ClassSchedule $schedule): void
    {
        if ($schedule->isDirty('time_start') || $schedule->isDirty('time_end')) {
            $oldUnits = self::calculateQuotaUnits(
                $schedule->getOriginal('time_start'),
                $schedule->getOriginal('time_end')
            );
            $newUnits = self::calculateQuotaUnits(
                $schedule->time_start,
                $schedule->time_end
            );

            // 5 || 2 || 3  = 2 jam
            // 5 || 3 (2 - 3) = -1 || 3 ( 3 + (-1)) = 2  = 3 jam
            // 5 || 3 (2 - 4) = 2 || 3 ( 3 + (-2)) = 1  = 4 jam
            // 5 || 3 (2 - 1) = 1 || 3 ( 3 + 1) = 4  = 1 jam
            if ($oldUnits != $newUnits) {
                $package = StudentPackage::where([
                ...
                if (!$package) return;
                # code...
                $actualUnits              = $oldUnits - $newUnits;
                $package->remaining_quota = $package->remaining_quota + $actualUnits;
                $package->used_quota      = $package->used_quota - $actualUnits;
                // 👇 FLAG PENTING diaktifkan
                $package->skip_quota_recalculation_cancel = true;

                // 2. 🛑 SOLUSI KUNCI: HAPUS FLAG DARI ATTRIBUTES SEBELUM SAVE
                // Ini memastikan Eloquent tidak melihatnya sebagai kolom yang perlu di-UPDATE
                unset($package->attributes['skip_quota_recalculation_cancel']);
    ...
    protected static function handleCancellationQuotaReturn(ClassSchedule $schedule): void
    {
        ...
        if ($schedule->isDirty('status') && $schedule->status === 'cancelled') {
            ...
            // dd($package, $package->remaining_quota + $oldUnits , $package->used_quota - $oldUnits);

            $package->remaining_quota = $package->remaining_quota + $oldUnits;
            $package->used_quota      = $package->used_quota - $oldUnits;
            // 👇 FLAG PENTING diaktifkan
            $package->skip_quota_recalculation_cancel = true;
    ...
    protected static function deductInitialQuota(ClassSchedule $schedule): void
    {
        ...
        $package = StudentPackage::where('student_id', $schedule->student_id)
            ...
            ->where([
                ['remaining_quota', '>=', $quotaUnitsNeeded], // ✅ Array bersarang untuk operator
                ['status', '=', 'active'], // Anda juga bisa menggunakan array untuk kondisi status
            ])
        ...
        // dd(
        //     'quotaUnitsNeeded', $quotaUnitsNeeded,
        //     'Paket Siswa (Package):', $package,
        //     'remaining_quota:', $package->remaining_quota ,
        //     'used_quota:', $package->used_quota ,
        ...
        $package->skip_quota_recalculation_create = true;

            // 2. 🛑 SOLUSI KUNCI: HAPUS FLAG DARI ATTRIBUTES SEBELUM SAVE
            // Ini memastikan Eloquent tidak melihatnya sebagai kolom yang perlu di-UPDATE
        unset($package->attributes['skip_quota_recalculation_create']);
        $package->save();
    }
